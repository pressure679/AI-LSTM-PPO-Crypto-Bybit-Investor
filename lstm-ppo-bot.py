...
yf_symbols = ["BNB-USD", "ETH-USD", "XRP-USD"]
...
def calc_order_qty(risk_amount: float,
    ...
    # print(f"risk amount: {risk_amt}, price: {price}")
...
def calculate_position_size(balance, risk_pct, sl_dist, leverage, min_qty):
    ...
    if sl_dist < 1:
        position_size = risk_amount / (sl_dist * price)
        position_size = position_size / 3.76
...
def train_bot(df, agent, symbol, bybit_symbol, window_size=20):
    ...
    for t in range(window_size, len(df) - 1):
        ...
        position_size = calc_order_qty(invest * leverage, price, min_qty, qty_step)

        # position_size = invest
        tp_dist = round(price * 0.05, price_precision)
        sl_dist = round(price * 0.02, price_precision)
        ...
        current_day = str(df.index[t]).split(' ')[0]
        ...
        if day != current_day:
            ...
            print(f"[{symbol}] Day {current_day} - Trades: {daily_trades} - Avg Profit: {avg_profit_per_trade:.2f}, {(avg_profit_per_trade/(position_size * price))*1000:.2f}% - PnL: {daily_pnl/capital*100:.2f}% - Balance: {capital:.2f} - Sharpe: {sr:.2f} - Sortino: {sor:.2f}")
            process_counter += 1
        ...
        if position == 0:
            if action == 1 and knn.predict_win_rate(state_seq) > 0.9 and df['signal'].iloc[t] == 1:
                ...
                invest = capital * 0.05
                # position_size = invest
                # print(f"entry_price: {entry_price}, price: {price}")
                position_size = calc_order_qty(invest * leverage, entry_price, min_qty, qty_step)
                # position_size = invest * leverage / sl_dist
                # position_size = calculate_position_size(capital, 0.05, sl_dist, leverage, min_qty)
                # print(f"[{bybit_symbol}] position_size: {position_size}")
                # leverage = get_max_leverage(position_size, bybit_symbol)
                # position_size = calculate_position_size(capital, 0.05, sl_dist, leverage, min_qty)
                entry_price = price
                capital -= position_size * entry_price * 0.00075
                position = 1
        ...
        if position == 1:
            ...
            if price <= (min_price + position_size * invest * 0.00075 * 2 + position_size * 0.00025) * (1 - trailing_sl_pct):
                action = 3
            ...
            if price >= (max_price - position_size * invest * 0.0075 * 2 - position_size * 0.00025) * (1 + trailing_sl_pct):
                action = 3
                trailing_sl_hit = True
            # tp_levels = [
            #     entry_price - 0.382 * tp_dist,
        ...
        if action == 3 and df['signal'].iloc[t] == -1 and position == 1 and entry_price != 0:
            ...
            capital -= position_size * entry_price * 0.00075
            pnl = (position_size * entry_price * final_pct - position_size * entry_price * 0.00075 * 2 - 0.00025 * position_size * entry_price) * 20 / 800
            # print(f"profit pct: {final_pct}, pnl: {pnl}")
            capital += pnl
        ...
        if action == 0 and position != 0:
            ...
            pnl = position_size * entry_price * profit_pct
            if pnl <= -invest:
                action == 3
...
def test_bot(df, agent, symbol, bybit_symbol, window_size=20):
    ...
    while True:
        ...
        response = session.get_positions(category="linear", symbol=bybit_symbol)
        # print(f"session.get_positions() response: {response}")
        ...
        if positions:
            ...
            if 'avgPrice' in pos and pos['size'] != '0':
                ...
                entry_price = 0.0
            # print(f"in 'if positions:', session_position: {session_position}, position: {position}")
        ...
        if current_day is None:
            ...
            print(f"[{symbol}] Day {current_day} - Trades: {daily_trades} - Avg Profit: {avg_profit_per_trade:.2f}, {(avg_profit_per_trade/(position_size*price))*100:.2f}% - PnL: {daily_pnl/capital*100:.2f}% - Balance: {capital:.2f} - Sharpe: {sr:.2f} - Sortino: {sor:.2f}")
            current_day = day
        ...
        if position == 0:
            ...
            if action == 1 and knn.predict_win_rate(state_seq) > 0.9 and df['signal'].iloc[-1] == 1:
                ...
                capital = get_balance()
                invest = capital * 0.05
                # position_size = invest * 4
                # print(f"invest: {invest}, entry_price: {entry_price}")
                position_size = calc_order_qty(invest * leverage, entry_price, min_qty, qty_step)
                # position_size = calculate_position_size(capital, 0.05, sl_dist, leverage, min_qty)
        ...
        if action == 3 and position == 1 and df['signal'].iloc[-1] == -1 and entry_price != 0:
            final_pct = (price - entry_price) / entry_price if position == 1 else (entry_price - price) / entry_price
            reward += final_pct - 0.00075 * 2 - 0.00025
            daily_pnl += profit_pct * position_size * entry_price - position_size * entry_price * 0.00075 * 2 - position_size * entry_price * 0.00025
            knn.add(entry_state, is_win=(final_pct - 0.00075 * 2 - 0.00025 > 0.00))
