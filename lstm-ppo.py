...
def train_bot(df, agent, symbol, window_size=20):
    ...
    for t in range(window_size, len(df) - 1):
        ...
        if action == 1 and position == 0:  # Buy
            ...
            if df['macd_signal_diff'].iloc[t] > 0:
                reward += 1
            else:
                reward -= 1
            ...
            if capital > peak_capital:
                ...
                reward -= drawdown * 10  # e.g., 5% drawdown → reward -= 0.05
                invest *= (1 + drawdown * 10)
...
def test_bot(df, agent, symbol, bybit_symbol, window_size=20):
    ...
    while True:
        ...
        if action == 1 and position == 0:  # Buy
            ...
            print("Entered buy order ({bybit_symbol})")
            ...
            print("Entered sell order ({bybit_symbol})")
            entry_price = current_price
            ...
            print("Closed order ({bybit_symbol})")
            position = 0
            invest = 0
...
def main():
    ...
    if train:
        for symbol in symbols:
            ...
            if symbol == "XAUUSD":
                # t0 = threading.Thread(target=load_last_mb_xauusd, args=())
                # t0.start()
                # test_threads.append(t0)
                df = load_last_mb_xauusd("/mnt/chromeos/removable/sd_card/XAUUSD_1m_data.csv")
            else:
                # t1 = threading.Thread(target=train_bot, args=(df, lstm_ppo_agent, symbol))
                # t1.start()
            ...
            test_threads[counter] = threading.Thread(target=train_bot, args=(df, lstm_ppo_agent, symbol))
            test_threads[counter].start()
            thread_counter += 1
            # t2 = threading.Thread(target=train_bot, args=(df, lstm_ppo_agent, symbol))
            # test_threads.append(t2)
    ...
    for bybit_symbol in bybit_symbols:
        ...
        test_threads[thread_counter] = threading.Thread(target=test_bot, args=(df, lstm_ppo_agent, symbols[counter], bybit_symbol))
        test_threads[thread_counter].start()
        thread_counter += 1
        # test_threads.append(t3)
